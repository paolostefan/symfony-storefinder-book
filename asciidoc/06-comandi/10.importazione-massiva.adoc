...
Per tornare a questo punto del libro potete fare _checkout_ del tag #NON ESISTE ANCORA#. (((Git, git checkout)))
...
Per creare un nuovo comando, si può usare il comando (non è un gioco di parole icon:smile-o[]) `make:command`.
...
Funziona? Ok, allora modifichiamolo! icon:smile-o[]
...
====
Forse avete notato che negli screenshot il comando non è `poi:import` ma `poi:`. + 
