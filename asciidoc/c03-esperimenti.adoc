...
****
In questo corso i percorsi dei file -salvo dove specificato diversamente- vanno sempre intesi come _relativi_ alla directory principale del progetto Symfony (che si chiama *guybrush*, se finora avete seguito le istruzioni alla lettera). Ad esempio: se troverete scritto "apriamo il file *config/route.yaml*", saprete che si tratta del file *route.yaml* nella sottodirectory *config* del progetto Symfony.
****
