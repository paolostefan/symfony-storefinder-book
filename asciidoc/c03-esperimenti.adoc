...
****
I percorsi dei file sono _relativi_ alla directory principale del progetto Symfony (che si chiama *guybrush*, se finora avete seguito le istruzioni alla lettera). Ad esempio: se troverete scritto "apriamo il file *config/route.yaml*", saprete che si tratta del file *route.yaml* nella sottodirectory *config* del progetto Symfony.
****
...
. Dal vostro PC fate il checkout del ramo *empty_project*.
+
[source,bash]
----
...
// Per tornare a questo punto del libro potete fare checkout del ramo *sample_controllers* del repository. (((Git, git checkout)))
